...
class Scene11_1_TelemetryTruth(Scene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(scenario), run_time=0.8)
        ...
        self.play(FadeIn(telemetry), run_time=0.8)
        ...
        self.play(Create(bad_box), FadeIn(bad_label), run_time=0.8)
...
class Scene11_2_KalmanFilter(Scene):
    ...
    def construct(self):
        ...
        self.play(Write(state), FadeIn(state_labels), run_time=1)
        ...
        self.play(FadeIn(noisy_dots), FadeIn(noisy_label), run_time=0.8)
        ...
        self.play(Create(filtered_line), FadeIn(filtered_label), run_time=1.5)
...
class Scene11_3_KalmanEquations(Scene):
    ...
    def construct(self):
        ...
        self.play(Write(predict_label), Write(predict), run_time=1)
        ...
        self.play(Write(update_label), Write(update), run_time=1)
        ...
        self.play(Write(gain_label), Write(gain), run_time=1)
...
class Scene11_4_BrakingAggression(Scene):
    ...
    def construct(self):
        ...
        self.play(Write(jerk_def), FadeIn(jerk_label), run_time=1)
        ...
        self.play(Write(bai), Create(bai_box), run_time=1)
        ...
        y_label = Text("Decel (m/s²)", font_size=12, color=TEXT_GRAY)
        ...
        self.play(
            Create(smooth_line), FadeIn(smooth_label),
            Create(aggro_line), FadeIn(aggro_label),
            run_time=1.5
        )
...
class Scene11_5_FrictionCircle(Scene):
    ...
    def construct(self):
        ...
        # g-g diagram
        circle = Circle(radius=2, color=ELO_GOLD, stroke_width=2)
        circle.move_to(DOWN * 1)
        
        # Axes through center
        h_line = Line(LEFT * 2.5, RIGHT * 2.5, color=TEXT_GRAY)
        h_line.move_to(circle.get_center())
        v_line = Line(DOWN * 2.5, UP * 2.5, color=TEXT_GRAY)
        v_line.move_to(circle.get_center())
        
        # Labels
        brake_label = Text("Brake", font_size=14, color=TEXT_GRAY)
        brake_label.move_to(circle.get_center() + DOWN * 2.3)
        
        accel_label = Text("Accelerate", font_size=14, color=TEXT_GRAY)
        accel_label.move_to(circle.get_center() + UP * 2.3)
        
        left_label = Text("Left", font_size=14, color=TEXT_GRAY)
        left_label.move_to(circle.get_center() + LEFT * 2.5)
        
        right_label = Text("Right", font_size=14, color=TEXT_GRAY)
        right_label.move_to(circle.get_center() + RIGHT * 2.5)
        ...
        radii = np.random.uniform(1.5, 1.98, 30)  # Close to limit
        ...
        self.play(Write(skill_note), run_time=1)
...
class Scene11_3a_ExplainKalmanCycle(Scene):
    ...
    def construct(self):
        ...
        self.play(FadeIn(predict_box), run_time=1)
        self.play(Create(arrow), FadeIn(arrow_label), run_time=0.5)
        self.play(FadeIn(update_box), run_time=1)

        # Feedback loop arrow
        loop_arrow = CurvedArrow(
            update_box[0].get_bottom() + RIGHT * 0.5,
            predict_box[0].get_bottom() + LEFT * 0.5,
            color=ELO_PURPLE, angle=TAU/4
        )
        ...
        self.play(Write(kg_formula), run_time=1)
        ...
        self.play(FadeIn(kg_explain), run_time=1)
...
class Scene11_5a_ExplainFrictionCircle(Scene):
    ...
    def construct(self):
        ...
        self.play(Create(x_ax), Create(y_ax), FadeIn(fx_label), FadeIn(fy_label), run_time=0.5)
        ...
        self.play(Create(force_vec), FadeIn(force_label), run_time=0.5)
        ...
        # Key insight
        insight = VGroup(
            Text("All braking (F_x = μN) → F_y = 0 → can't turn!", font_size=16, color=ELO_RED),
            Text("All cornering (F_y = μN) → F_x = 0 → can't brake!", font_size=16, color=ELO_BLUE),
